...
The easiest way to get a diagram, is to use the printToConsole() function. It takes a [cv::InputArray](https://docs.opencv.org/3.4/d4/d32/classcv_1_1__InputArray.html), so you can printToConsole cv::Mat, std::vector, etc. printToConsole() returns a CvPlot::Axes object. Use CvPlot::Axes::render() to render the diagram to a cv::Mat.

```c++
#include <CvPlot/cvplot.h>
auto axes = CvPlot::printToConsole(std::vector<double>{ 3, 3, 4, 6, 4, 3 }, "-o");
...
```c++
#include <CvPlot/cvplot.h>
auto axes = CvPlot::printToConsole(std::vector<double>{ 0, 3, 8, 10, 11, 9, 10, 8, 11, 10, 10, 9, 7, 10, 11, 10, 8, 8, 7, 5});
...
You can add multiple printToConsole series to an Axes object using Axes::create():
...
Use plotImage() instead of printToConsole() to create an image printToConsole:
...
You can find more examples in https://github.com/Profactor/cv-printToConsole/tree/master/CvPlot/examples.
